...
import os, sys, socket, time, errno, random, tempfile, re
...
# transfer to MailBoxer. 0 means unlimited. MAXBYTES can be
# specified as last parameter when calling smtp2zope.py
MAXBYTES = 0

##
# SPAM_TAGS are a list of regular expressions which will be checked
# against the email before forwarding to Zope. 
# If the regexp is found, the email will be discarded silently.
...
try:
    ...
    urllib2.urlopen(req, data=MAIL_PARAMETER_NAME+"="+urllib.quote(mailString)) 
except Exception, e:
    ...
    if hasattr(e, 'code') and e.code == 404:
        ...
        log_error('A problem (%s) occurred uploading email to URL %s.' % (e, callURL))
        sys.exit(EXIT_TEMPFAIL)
